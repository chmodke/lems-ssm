...
<script>
    $(function () {
        ...
        var vali_flag = false;
        $("#rep_user_passwd_btn").click(rep_user_passwd);//添加用户按钮事件绑定
        ...
        function vali_valiCode() {
            if (user_flag) {
                ...
                $.ajax({
                    ...
                    type: "post",
                    data: {"valiCode": rep_user_pwd_vali_code},
                    dataType: "json",
                    success: function (result) {
                        if (result.status == 0) {
                            vali_flag = true;
        ...
        function rep_user_passwd() {
            ...
            if (vali_passwd_box() && vali_flag && user_flag) {
                ...
                var rep_user_passwd_msg = Base64.encode(rep_user_passwd_msg);
                ...
                $.ajax({
                    ...
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader("Authorization_rep_user_passwd", "Basic " + rep_user_passwd_msg);
                    },
                    success: function (result) {
                        if (result.status == 0) {
                            $.messager.alert('提示', result.message);
//                            rep_user_passwd_reset();
    ...
    <form method="post">
        <table class="table table-hover table-condensed" style="width: 500px;margin: auto">
            ...
            <tr>
                <th>重复密码</th>
                <td>
                    <input id="rep_user_passbox2" type="password" placeholder="请输入密码" class="easyui-validatebox"
                           data-options="required:true">
                </td>
                <td style="width:130px;margin-left: 80px">
